// 🎮 MAZE RUNNER – versión con sprites, sonidos y términos
...
function drawPlayer() {
  ...
  ctx.drawImage(sprite, playerX * cellSize - offset, playerY * cellSize - offset, cellSize * scale, cellSize * scale);
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (["w","W"].includes(e.key)) newY--, playerDirection = "up";
  else if (["s","S"].includes(e.key)) newY++, playerDirection = "down";
  else if (["a","A"].includes(e.key)) newX--, playerDirection = "left";
  else if (["d","D"].includes(e.key)) newX++, playerDirection = "right";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const botonesPersonajes = document.querySelectorAll("#character-select button");
  botonesPersonajes.forEach((boton, i) => {
    boton.addEventListener("click", () => {
      personajeSeleccionado = i + 1;
